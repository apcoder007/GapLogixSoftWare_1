...
function apply_select2() {
	$('select:not(.not_appli)').select2().on("opening", function() {
...
function timepicker_onclose(id) {
	...
	if (val != null && val != '') {
		$.post(base_url + "/index/gettimeformat", {
			sel_time: val
		}, function(data) {
...
function disp_requisition(val, disp_id) {
	...
	$('#' + disp_id).val('');
	if (val != '') {
		$.post(base_url + "/employee/getemprequi", {
			cand_id: val
		}, function(data) {
...
function getEmailOfUser(obj, email_id) {
	...
	if (cand_id != '') {
		$.post(base_url + "/usermanagement/getemailofuser", {
			cand_id: cand_id
		}, function(data) {
	...
	$('#formid').ajaxForm({
		...
		success: function(response, status, xhr) {
				...
				if (response['result'] != 'saved') {
					$.each(this, function(k, v) {
						if (elementid != id) {
							...
							if ($("#" + id).length > 0) {
								if (formName == 'Changepassword') $("#" + id).parent().parent().append(getErrorHtml(v, id, '_' + id));
								else {
			...
			if ($(".errors").length > 0) {
				...
				if (varaible.substring(x + 1) == "registration") eval(jsFunction);
			}
			if (response['result'] == 'saved' || response['result'] == 'fbsaved' || response['result'] == 'exception') {
				if (toggleDivId.length > 2) $("#" + toggleDivId).toggle("slow");
				changepassworddefaultvalues();
				if (dialogMsg.length > 2) eval(dialogMsg);
				else eval(jsFunction);
				if (response['controller'] == 'pendingleaves') window.location.href = base_url + '/pendingleaves';
				if (response['nomessage'] != 'yes') {
					successmessage(response['message']);
				}
				if (response['page'] == 'changepassword') successmessagechange(response['message']);
			}
		}
	});
}
...
function successmessage_requeststatus(message, flag, controllername, url, flag_type) {
	...
	setTimeout(function() {
		$('#error_message_' + controllername).fadeOut('slow', function() {
			if (flag_type != 'dont_redirect') window.location = url;
		});
	}, 3000);
}
...
function changestatus(controllername, objid, flag) {
	...
	mdgdta = $.trim(msgdta);
	if (controllername == 'bgscreeningtype') var messageAlert = 'Are you sure you want to delete the selected screening type?';
	else if (controllername == 'businessunits') var messageAlert = 'Are you sure you want to delete the selected business unit?';
	else if (controllername == 'agencylist') var messageAlert = 'You are trying to delete the selected agency. The background check processes assigned to this agency will become invalid. Please confirm.';
	else if (controllername == 'pendingleaves') var messageAlert = 'Are you sure you want to cancel the selected leave request?';
	else if (controllername == 'roles') var messageAlert = 'Are you sure you want to delete the selected role name?';
	else if (controllername == 'categories') var messageAlert = 'Documents added to the selected category will also be deleted. Are you sure you want to delete the category?';
	else {
					...
					if (data.status == 'no') {
						...
						$.ajax({
							...
							success: function(response) {
								...
								if (response['flagtype'] == 'process') location.reload();
								else {
									if (deleteflag == 1) {
										redirecttocontroller(controllername);
									} else {
		...
		if (r == true) {
			$.ajax({
				...
				success: function(response) {
					...
					if (response['flagtype'] == 'process') location.reload();
					else getEmployeeAjaxgridData(controllername, userId);
				}
			});
		} else {}
	});
...
function change_request_status(controllername, objid, status, grid_type) {
	if (status == 'To approve') dispm = " send for approve ";
	else if (status == 'To manager approve') dispm = " send for manager approve ";
	else dispm = status.toLowerCase();
...
function getEmployeeAjaxgridData(objname, userId) {
	...
	if ($.inArray("mydetails", url) != -1) {
		...
		if ($.inArray("myemployees", url) != -1) context = 'myteam';
	...
	mname = '';
	mnuid = '';
	if (formGridId == '' || formGridId == 'undefined' || typeof(formGridId) === 'undefined') formGridId = '';
	else {
		unitId = formGridId.split("/");
		mname = unitId[0];
		mnuid = unitId[1];
	}
	if (page == '' || page == 'undefined' || typeof(page) === 'undefined') page = $(".currentpage").val();
	...
	if (searchData != '' && searchData != 'undefined') dataparam = dataparam + '&searchData=' + searchData;
	$('#searchdata').remove();
...
function getAjaxgridData(objname, dashboardcall) {
	...
	if (objname == 'servicerequests') {
		...
		if (typeof(v_val) == 'undefined') v_val = '';
	...
	if (objname == 'policydocuments') {
		...
		if (catid[1]) url = base_url + "/" + objname + "/id/" + catid[1];
	} else url = base_url + "/" + objname + "/index/format/html";
	$.ajax({
		url: url,
		type: 'POST',
		data: dataparam,
...
function redirecttolink(link, module) {
	...
	if (module != '') window.location = base_url + '/' + module + '/' + link;
	else window.location = base_url + '/' + link;
}
...
function closetab(ele, pagename, pagelink) {
	...
	jQuery.ajax({
		...
		beforeSend: function() {
			...
			$("#loader-recent").html("<img src=" + "/" + "adminfiles/images/loaderwhite_21X21.gif>");
		},
		success: function(response) {
			...
			if (typeof($('.recentviewd').find('ul li').html()) == 'undefined') $('#recentviewtext').html('');
		}
	}, 'json');
}
...
function paginationndsorting(url) {
	var myarr = url.split("/");
	if (url.indexOf('/call/ajaxcall') == -1) url = url + '/call/ajaxcall';
...
function refreshgrid(objname, dashboardcall, catId) {
	...
	if (catId != '') Url = base_url + "/" + objname + "/id/" + catId;
	else Url = base_url + "/" + objname + "/index/format/html";
...
function getsearchdata(objname, conText, colname, event, etype) {
	...
	if (perpage == 'undefined' || typeof(perpage) === 'undefined') {
		if (dashboardcall == 'Yes') perpage = '10';
		else perpage = '20';
	}
	var page = $(".gotopage_input_" + objname).val();
	var formGridId = $("#formGridId").val();
	...
	if (searchData != '' && searchData != '{}') dataparam = dataparam + '&searchData=' + searchData;
	if (objname == 'servicerequests') {
		var v_val = $('#service_grid_status').val();
		if (typeof(v_val) == 'undefined') v_val = '';
		dataparam = dataparam + '&t=' + $('#service_grid').val() + '&v=' + v_val;
	...
	$('#footer').append("<input type='hidden' value='" + searchData + "' id='" + objname + "_searchdata' />");
	$('#footer').append('<input type="hidden" value="' + objname + '" id="objectName" />');
	if ($("#columnId").length) $('#columnId').val(columnid);
...
function selectrow(objid, tr) {
	...
	if (!row.hasClass('newclass')) {
		row.addClass('newclass').siblings().removeClass('newclass');
	}
}
...
function getApprReqData(obj) {
	...
	if (rval != '') {
		$.post(base_url + "/default/requisition/getapprreqdata", {
			req_id: rval
		}, function(data) {
...
function getdepts_req(obj, dept_id, position_id) {
	var val = $(obj).val();
	$('#email_cnt').val('');
	$.post(base_url + "/default/requisition/getdepartments", {
		bunitid: val
	}, function(data) {
...
function getemail_cnt(bunit_id) {
	$('#email_cnt').val('');
	if (bunit_id != '') {
		$.post(base_url + "/default/requisition/getemailcount", {
			bunitid: bunit_id
		}, function(data) {
...
function bunit_emailcontacts(bunit_id) {
	...
	if (bunit != '') {
		...
		$.post(base_url + "/emailcontacts/getgroupoptions", {
			bunit: bunit
		}, function(data) {
...
function getStates_cand(country_id, state_id, city_id) {
	...
	$.post(base_url + "/default/states/getstatescand", {
...
function getcities_cand(state_id, city_id) {
	...
	$.post(base_url + "/default/cities/getcitiescand", {
		state_id: state_val
	}, function(data) {
		$('#' + city_id).find('option').remove();
...
function check_child_roles(menu_id, con) {
	...
	if (chk_status) {
		...
		if (con == '') {
			...
			if (menu_chk_len > 0) $('.cls_radiobuttons_div' + menu_id).show('slow', 'swing');
		}
		...
		if (con == '') $('.cls_radiobuttons_div' + menu_id).hide('slow', 'swing');
		...
		success: function(response) {
			if (typeof(response['result']) == 'undefined' || response['result'] == 'false' || response['result'] == '') {
				...
				if (menuname == 'City') {
					country_id = $('#country').val();
					state_id = $('#state').val();
					if (country_id == '' || typeof country_id == 'undefined') country_id = $('#country_1').val();
					if (country_id == '' || typeof country_id == 'undefined') country_id = $('#countryid').val();
					if (country_id == '' || typeof country_id == 'undefined') country_id = $('#perm_country').val();
					if (state_id == '' || typeof state_id == 'undefined') state_id = $('#state_1').val();
					if (state_id == '' || typeof state_id == 'undefined') state_id = $('#perm_state').val();
				...
				if (controllername == 'interviewrounds') {
					...
					if (url.indexOf('deptid') == -1) {
						if (deptid != '' && typeof(deptid) !== 'undefined') url = url + '/deptid/' + deptid;
					}
					...
					if (act_name == 'editpopup') {
						...
						{
							if (int_status == 'Completed') jAlert('The interview is completed for the candidate. So you cannot assign a new round to the candidate.');
							else jAlert('The candidate is not scheduled to the next round. So you cannot assign a new round to the candidate.');
							flag = 'no';
						} else {
...
function displaydeptform_frame(url, menuname) {
	...
	var flag = 'yes';
	var urlsplitArr = url.split("/");
	if (url.indexOf('http') != -1 || url.indexOf('https') != -1) var controllername = urlsplitArr[3];
	else var controllername = urlsplitArr[2];
	if (menuname == 'Position') {
		jobtitle_id = $('#jobtitle_id').val();
		if (jobtitle_id != null && jobtitle_id.length > 0) {
			job_title = jobtitle_id;
...
function closeiframepopup_03102013(controllername, con) {
	window.parent.$('#' + controllername + 'Container').dialog('close');
	if (con != 'cancel') parent.window.location.reload();
}
	...
	if (id != '') {
		...
		$.ajax({
			...
			beforeSend: function() {
				...
				$("#loader").html("<img src=" + "/" + "adminfiles/images/loaderwhite_21X21.gif>");
			},
			success: function(response) {
				if ($.trim(response) == 'nostates') {
					...
					if (con == 'otheroption') {
						...
						if ($('#' + eleId).parent().find("span.add-coloum").length > 0) $('#' + eleId).parent().find("span.add-coloum").prepend("<span class='errors' id='errors-" + eleId + "'>States are not configured yet.</span>");
						else $('#' + eleId).parent().append("<span class='errors' id='errors-" + eleId + "'>States are not configured yet.</span>");
				...
				if (response != '' && response != 'null' && $.trim(response) != 'nostates') {
					...
					$("#loader").remove();
					if ($("#errors-" + eleId).is(':visible')) $("#errors-" + eleId).hide();
					// $("#"+eleId).html(response);	 
					$("#" + eleId).html(responsedata);
					if ($("#otherstatediv").is(':visible')) $('#otherstatediv').hide();
					if (countryid != '') {
			...
			success: function(response) {
				if (flag == 'cand') {
					...
					if ($.trim(response.candidates) == 'nocandidates') {
						...
						$("#loader").remove();
						if ($("#errors-candidate_name").is(':visible')) $("#errors-candidate_name").hide();
					...
					if ($.trim(response.managers) == 'nomanagers') {
						...
						$("#loader").remove();
						if ($("#errors-interviewer_id").is(':visible')) $("#errors-interviewer_id").hide();
			...
			success: function(response) {
				if ($.trim(response) == 'nocities') {
					...
					if (con == 'otheroption') {
						...
						if ($('#' + eleId).parent().find("span.add-coloum").length > 0) $('#' + eleId).parent().find("span.add-coloum").prepend("<span class='errors' id='errors-" + eleId + "'>Cities are not configured yet.</span>");
						else $('#' + eleId).parent().append("<span class='errors' id='errors-" + eleId + "'>Cities are not configured yet.</span>");
						$("#" + eleId).find('option').remove();
				...
				if (response != '' && response != 'null' && $.trim(response) != 'nocities') {
					...
					console.log(responsedata);
					if (stateid == '') $('#s2id_' + eleId + ' .select2-choice span').html('Select city');
					$("#" + eleId).parent().find('.select2-container').find('.select2-search-choice').remove();
					$("#loader").remove();
					if ($("#errors-" + eleId).is(':visible')) $("#errors-" + eleId).hide();
					$("#" + eleId).html(responsedata);
...
function displayPasswordDesc(ele) {
	...
	if ($('[id^=password_]').html() != '') $('[id^=password_]').hide();
	$("#password_" + id).show();
}
...
function getPOCData(id, divnum) {
	...
	$.ajax({
		...
		success: function(response) {
			if (divnum == 0) $('.right-block-data').addClass('right-over-border');
			else $('.right-block-data').removeClass('right-over-border');
...
function from_to_date_validation_org(from_date_id, to_date_id, obj, message) {
	...
	if (from_val != '' && to_val != '') {
		...
		$("#loader").html("<img src=" + "/" + "adminfiles/images/loaderwhite_21X21.gif>");
		$.post(base_url + "/index/fromdatetodateorg", {
			from_val: from_val,
			to_val: to_val
		}, function(data) {
			if (data.result == 'no') {
...
function hidetodatecalender() {
	if ($("#fromdateerrorspan").is(":visible")) $("#fromdateerrorspan").hide();
	if ($("#errors-from_date").is(":visible")) $("#errors-from_date").hide();
	if ($("#errors-to_date").is(":visible")) $("#errors-to_date").hide();
	if ($("#todateerrorspan").is(":visible")) $("#todateerrorspan").hide();
...
function emptytodate(ele) {
	...
	if (fromdate <= y) {
		if (dayselected == 1) {
			...
			if (fromdateval != '') $("#appliedleavesdaycount").val(0.5);
			else $("#appliedleavesdaycount").val('');
		}
	} else {
		if (fromdate > y) {
			$("#" + selector).val('');
...
function validateselecteddate(ele) {
	...
	var selectorid = '';
	var selector = $(ele).prop('id');
	if (selector == 'from_date') selectorid = 1;
	else selectorid = 2;
			...
			success: function(response) {
				if (response['result'] == 'success' && response['result'] != '' && response['days'] != '') {
					...
					if (response['availableleaves'] != '' && response['days'] != '') {
						...
						if (response['days'] > response['availableleaves']) jAlert("Applied leaves exceed the available leaves count. Additional leaves will be considered as Loss of Pay.");
					}
				}
				...
				if (response['result'] == 'saved') {
					...
					setTimeout(function() {
						...
						if (response['commentcount'] == 'morethan2') {
							if ($("#commentopendiv").is(":visible")) $('#commentopendiv').css('display', 'block');
							else if ($("#commenthidediv").is(":visible")) $('#commenthidediv').css('display', 'block');
							else {
...
function displaycomments(detailId, con) {
	if (con == 'all') $('#commentopendiv').css('display', 'none');
...
function displayStatusdata(controllername) {
	...
	id = $("#statusid").val();
	if (id) {
		if (controllername == 'empscreening') window.location.href = base_url + '/' + controllername + '/con/' + id;
		else if (controllername == 'requisition') window.location.href = base_url + '/' + controllername + '/index/con/' + id;
		else window.location.href = base_url + '/' + controllername + '/' + id;
	}
}
	...
	if (id != '') {
		$.ajax({
			...
			beforeSend: function() {
				...
				$("#loader").html("<img src=" + "/" + "adminfiles/images/loaderwhite_21X21.gif>");
				$("#loader1").html("<img src=" + "/" + "adminfiles/images/loaderwhite_21X21.gif>");
			},
			success: function(response) {
				if (con == 'leavemanagement') {
					...
					if ($.trim($hrs) == 'nohrmanagers') {
						...
						if ($("#errors-hrmanager").is(':visible')) $("#errors-hrmanager").hide();
						//('#s2id_hrmanager').find('span').html('Select Hr Manager');
						$("#loader1").remove();
						$("#loader").remove();
						$("#hrmanager").html($hrs);
...
function displayReportingmanagers_emp(ele, eleId, role_id, empId) {
	...
	} else Url = base_url + "/employee/getempreportingmanagers/format/html";
	$("#" + eleId).find('option').remove();
...
function populateCurrentAddress(ele) {
	if ($('#address_flag').is(":checked")) {
		...
		if (street_address != '' && country != '' && state != '' && city != '' && pincode != '') {
			...
			if ($("#errors-address_flag").is(':visible')) $("#errors-address_flag").hide();
		...
		$.post(base_url + "/index/fromdatetodate", {
			...
			if (data.result == 'no') {
				...
				$.ajax({
					...
					success: function(response) {
						$("#loader").remove();
						if (from_date_id == "leavebyemp_from_date") $("#leavebyemp_days").val(response);
						else if (from_date_id == "empleave_from_date") $("#empleave_days").val(response);
						else $('#dependent_age').val(response);
					}
				});
			}
		}, 'json');
...
function gobacktocontroller(context) {
	if (context == 'view' || context == 'edit') window.location.href = base_url + '/employee';
	else if (context == 'myemployees' || context == 'myemployeesedit') window.location.href = base_url + '/myemployees';
	else window.location.href = base_url + '/mydetails/edit';
}
...
function makeActiveInactive_1(status, emp_id) {
	if (status == 'other') {
		var empstatus = '';
		if (emp_id == 2) empstatus = 'Resigned';
		else if (emp_id == 3) empstatus = 'Left';
		else if (emp_id == 4) empstatus = 'Suspended';
		jAlert('You cannot activate / inactivate an employee when the employement status is ' + empstatus + '', 'Alert');
	} else {
		if (status == 'inactive') var mstatus = 'inactivate this employee';
		else mstatus = 'activate this employee';
...
function validatecost() {
	if ($("#errors-amountvalidation").is(":visible")) $("#errors-amountvalidation").hide();
...
function createorremoveshortcut(menuid, shortcutflag) {
	var actionvar = '';
	var html = '';
	if (shortcutflag == 1) actionvar = 'pin to shortcuts';
	else if (shortcutflag == 2) actionvar = 'unpin from shortcuts';
	else if (shortcutflag == 3) actionvar = 'pin to shortcuts';
	if ($("#errors-pageshortcut").is(":visible")) $("#errors-pageshortcut").hide();
	$.ajax({
		...
		beforeSend: function() {
			...
			$("#loader-shortcut").html("<img src=" + "/" + "adminfiles/images/loaderwhite_21X21.gif>");
		},
		success: function(response) {
...
function validateorgstartdate(ele, con, bunitid) {
	...
	var errormsg;
	if (con == '') con = 'organisationinfo';
	if ($(".errors").is(":visible")) $(".errors-" + ele.id).remove();
	if (startdate != '') {
		$.ajax({
			...
			dataType: 'json',
			beforeSend: function() {
				if (con == 'organisationinfo') $("#org_startdate").before("<div id='loader'></div>");
				else $("#start_date").before("<div id='loader'></div>");
				$("#loader").html("<img src=" + "/" + "adminfiles/images/loaderwhite_21X21.gif>");
			},
			success: function(response) {
				$("#loader").remove();
				if (response['result'] == 'error' && response['result'] != '') {
					if (con == 'organisationinfo') {
						...
						if (bunitid != '') errormsg = "Business unit start date must be less than Department start date and greater than Organization start date.";
						else errormsg = "Business unit start date must be greater than Organization start date.";
						$("#start_date").parent().append('<span id="errors-start_date" class="errors">' + errormsg + '</span>');
						$("#start_date").val('');
					} else if (con == 'departments') {
						if (bunitid > 0) errormsg = "Department start date must be greater than Business unit and Organization start dates.";
						else errormsg = "Department start date must be greater than Organization start date.";
						$("#start_date").parent().append('<span id="errors-start_date" class="errors">' + errormsg + '</span>');
						$("#start_date").val('');
					}
...
function validateorgandunitstartdate(ele, con) {
	...
	if ($(".errors").is(":visible")) $(".errors").remove();
...
function changereportsscreen(controllername) {
	if (controllername) window.location.href = base_url + '/reports/' + controllername;
}
...
function downloadBUsPdf(url, formId) {
	...
	if (data == '') var data = $(formId).serialize();
	$.ajax({
		type: "POST",
		url: url,
		data: data,
		...
		$.ajax({
			...
			success: function(response) {
				...
				if ($.trim(response) == 'noemployees') {
					if (elementid == 'approvingauthority') $('#s2id_approver_1').parent().parent().append("<span class='errors' id='errors-approver_1'>No employees found.</span>");
					else if (elementid == 'approver_1') $('#s2id_approver_2').parent().parent().append("<span class='errors' id='errors-approver_2'>No employees found.</span>");
					else if (elementid == 'approver_2') $('#s2id_approver_3').parent().parent().append("<span class='errors' id='errors-approver_3'>No employees found.</span>");
				}
				if (response != '' && response != 'null' && $.trim(response) != 'noemployees') {
					if (elementid == 'approvingauthority') {
						...
						$('#s2id_approver_2 .select2-choice span').html('Select Approver 2');
						$('#s2id_approver_3 .select2-choice span').html('Select Approver 3');
						if (value == 2) $("#approver_2").append("<option value='' label='Select Approver 2'>Select Approver 2</option>");
...
function approvedrejectRequisition($flag, $req_id) {
	if ($flag == 2) var mdgdta = 'Approve';
	else var mdgdta = 'Reject';
